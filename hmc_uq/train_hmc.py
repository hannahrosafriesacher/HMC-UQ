...
for chain in range(nr_chains):
    ...
    nuts_kernel = NUTS(net.to(device), jit_compile=True)
    mcmc = MCMC(nuts_kernel, num_samples=nr_samples, warmup_steps=warmup_steps, num_chains=nr_chains)

    # Run MCMC
    mcmc.run(x_train.float().to(device), y_train.float().to(device))
    ...
    #get predictions for validation ds
    pred_val = predictive(x_val)['obs']
    preds_chains_val.append(pred_val)

    if evaluate_testset:
        pred_te = predictive(x_te)['obs']
